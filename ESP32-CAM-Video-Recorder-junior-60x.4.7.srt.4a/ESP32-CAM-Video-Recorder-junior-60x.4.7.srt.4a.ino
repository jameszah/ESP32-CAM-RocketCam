/*
  ...
  ESP32-CAM-ESP32-CAM-RocketCam

  https://github.com/jameszah/ESP32-CAM-RocketCam

  This program records an mjpeg avi video to the sd card of an ESP32-CAM, with sensor for altitude, temperature, and acceleration from
  a bmp-180 and adxl345 sensor, storing the sensor readings as text captions in an srt file alongside the avi.

  The is a fork of https://github.com/jameszah/ESP32-CAM-Video-Recorder-junior with the sensor stuff added

  by James Zahary Aug 28, 2022
     jamzah.plc@gmail.com

   This is the junior hisory:
   ...
   - 58.9 apr 29,2022 - re-connect wifi after failure
    ...
    -v60.4 jun 15,2022 - two channels of streaming on port 81 and 82 to kept webpage working on port 80
     ...
     60.4.5  - semaphore and framebuffer2/3 when using capture sytle video streaming
          ...
          .7  - every 15
          .srt.1 - add bmp180, adxl345 and srt subtitles
               .2
               .3 grove
               .4 adafruit sep file
                  un-workaround dst bug
                  https://github.com/espressif/arduino-esp32/issues/6786
                
                .4a print sensors in ft and ft/s2

    https://github.com/jameszah/ESP32-CAM-RocketCam is licensed under the
  ...
  - Adafruit ADXL345 Library by Adafruit Version 1.3.1
  - Adafruit BMP085 Unified by Adafruit Version 1.1.1
  - Adafruit BusIO by Adafruit Version 1.13.2
  - Adafruit Unified Sensor by Adafruit Version 1.1.6

  Compiled with Arduino 1.8.19, and esp32-arduino core version 2.0.4, on Aug 28, 2022

    Successfully created esp32 image.
    "C:\\ArduinoPortable\\arduino-1.8.19\\portable\\packages\\esp32\\hardware\\esp32\\2.0.4/tools/gen_esp32part.exe" -q "C:\\Users\\James\\AppData\\Local\\Temp\\arduino_build_200766/partitions.csv" "C:\\Users\\James\\AppData\\Local\\Temp\\arduino_build_200766/ESP32-CAM-Video-Recorder-junior-60x.4.7.srt.4.ino.partitions.bin"
    ...
    Using library Wire at version 2.0.0 in folder: C:\ArduinoPortable\arduino-1.8.19\portable\packages\esp32\hardware\esp32\2.0.4\libraries\Wire
    Using library Adafruit_Unified_Sensor at version 1.1.6 in folder: C:\ArduinoPortable\sketch\libraries\Adafruit_Unified_Sensor
    Using library Adafruit_BMP085_Unified at version 1.1.1 in folder: C:\ArduinoPortable\sketch\libraries\Adafruit_BMP085_Unified
    Using library Adafruit_ADXL345 at version 1.3.1 in folder: C:\ArduinoPortable\sketch\libraries\Adafruit_ADXL345
    Using library Adafruit_BusIO at version 1.13.2 in folder: C:\ArduinoPortable\sketch\libraries\Adafruit_BusIO
    Using library SPI at version 2.0.0 in folder: C:\ArduinoPortable\arduino-1.8.19\portable\packages\esp32\hardware\esp32\2.0.4\libraries\SPI
    "C:\\ArduinoPortable\\arduino-1.8.19\\portable\\packages\\esp32\\tools\\xtensa-esp32-elf-gcc\\gcc8_4_0-esp-2021r2-patch3/bin/xtensa-esp32-elf-size" -A "C:\\Users\\James\\AppData\\Local\\Temp\\arduino_build_200766/ESP32-CAM-Video-Recorder-junior-60x.4.7.srt.4.ino.elf"
    Sketch uses 1055381 bytes (33%) of program storage space. Maximum is 3145728 bytes.
    Global variables use 55080 bytes (16%) of dynamic memory, leaving 272600 bytes for local variables. Maximum is 327680 bytes.
...
static const char vernum[] = "v60.4.7.srt.4";
char devname[30];
...
#define fbs 2 // 8 // was 64 -- how many kb of static ram for psram -> sram buffer for sd write
...
static void start_avi() {
  ...
  sprintf(srt_file_name, "/%s%d.%03d.srt",  devname, file_group, file_number);

  file_number++;

  avifile = SD_MMC.open(avi_file_name, "w");
  idxfile = SD_MMC.open("/idx.tmp", "w");
  srtfile = SD_MMC.open(srt_file_name, "w");
  ...
  if (srtfile)  {
    ...
    Serial.println("Could not open file srt file ");
    major_fail();
  }
...
void setup() {
  ...
  Serial.println("                                    ");
  Serial.println("-------------------------------------");
  Serial.printf("ESP32-CAM-RocketCam %s\n", vernum);
  Serial.println("-------------------------------------");
  ...
  xTaskCreatePinnedToCore( the_sd_loop, "the_sd_loop", 6000, NULL, 4, &the_sd_loop_task, 1);  // prio 4, core 1 2000->3000
...
void the_sd_loop (void* pvParameter) {
  ...
  while (1) {
    ...
    if (millis() > next_srt_millis && srtfile) {
      next_srt_event++;
      srtfile.println(next_srt_event);
      end_srt = millis() - avi_start_time;

      srtfile.printf("%02d:%02d:%02d,%03d -->  %02d:%02d:%02d,%03d\n",
                     (int) ((beg_srt / (1000 * 60 * 60)) % 24),
                     (int) ((beg_srt / (1000 * 60)) % 60),
                     (int) (beg_srt / 1000) % 60,
                     (int) (beg_srt % 1000),
                     (int) ((end_srt / (1000 * 60 * 60)) % 24),
                     (int) ((end_srt / (1000 * 60)) % 60),
                     (int) (end_srt / 1000) % 60,
                     (int) (end_srt % 1000));
      time_t now ;
      timeinfo = { 0 };
      ...
      if (adxl345) {
        ...
        get_adxl(altitude, temperature);
        //srtfile.printf("%.1fm  %.1fc ", altitude, temperature );
        srtfile.printf("%.1f ft  %.1f f ", altitude * 3.28084, temperature  * 1.8  + 32);
        
        //Serial.printf("%.1fm  %.1fc \n", altitude, temperature);
      }
      if (bmp180) {
        float fx, fy, fz;
        get_bmp180(fx, fy, fz);
        ...
        //Serial.print("Z: "); Serial.print(fz); Serial.print("  "); Serial.println("m/s^2 ");
        //srtfile.printf("X:%.1f Y:%.1f Z:%.1f  (m/s^2)\n", fx, fy, fz);
        srtfile.printf("x:%.1f y:%.1f z:%.1f  (ft/s^2)\n", fx * 3.28084, fy * 3.28084, fz * 3.28084);
      }
      
      strftime(strftime_buf, sizeof(strftime_buf), "%F %H:%M:%S", &timeinfo);

      srtfile.printf("%02d:%02d:%02d  ",
                     (int) ((beg_srt / (1000 * 60 * 60)) % 24),
                     (int) ((beg_srt / (1000 * 60)) % 60),
                     (int) (beg_srt / 1000) % 60);

      srtfile.println(strftime_buf);

      srtfile.printf("\n");

      beg_srt = end_srt; 
      next_srt_millis = millis() + 500;
    }
...
void the_camera_loop (void* pvParameter) {
  ...
  while (1) {
    ...
    if (frame_cnt == 0 && start_record == 0) {
      ...
      beg_srt = avi_start_time;
      next_srt_millis = avi_start_time + 500;
      next_srt_event = 0;
